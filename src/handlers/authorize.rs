use crate::{
    config::Config,
    handlers::{
        provider::{FacebookProvider, GithubProvider, GoogleProvider, Provider, ProviderResponse, ProviderState},
        Error,
    },
};
use log::error;
...
pub fn authorize(config: State<Config>, provider: String) -> ProviderResponse {
    ...
    return ProviderResponse::Redirect(Redirect::to(String::from(auth_url)));
}
